...
def index(request):
    ...
    if flag:
        ...
        return render(request, 'client/client_index.html',
                      {'username': username, 'flag': flag, 'c_num': c_num, 'classes_list': classes_list,
    ...
    if "login_user" in request.session:
        ...
        for brand in brands_list:
            if brand['isdesign'] == u'是':
                brands_list_design.append(brand)
        return render(request, 'client/client_index.html',
                      {'message': '<script type="text/javascript">alert("你都没有登录，还注销个毛线吖，尼码！");</script>',
                       'classes_list': classes_list, 'brands_list': brands_list,
                       'brands_list_design': brands_list_design})
...
def add_cart(request):
    ...
    if flag:
        ...
        commodity = mongodb_options.find_commodity_by_cid(db, commodity_id)
        commodity_dict = {'commodity_id': commodity_id, 'c_name': commodity['c_name'], 'num': 1, 'color': color,
                          'size': size, 'unit_price': commodity['c_price']}
        ...
        return render(request, 'client/client_cart_info.html',
                      {'username': username, 'flag': flag, 'c_num': c_num, 'commodity_id': commodity_id,
                       'classes_list': classes_list, 'brands_list': brands_list,
                       'brands_list_design': brands_list_design})
    else:
        classes_list = mongodb_options.find_classes(db)
        ...
        for brand in brands_list:
            if brand['isdesign'] == u'是':
                brands_list_design.append(brand)
        return render(request, 'client/client_index.html',
                      {'message': '<script type="text/javascript">alert("你都没有登录，还看个毛线吖，尼码！");</script>', 'flag': flag,
                       'classes_list': classes_list, 'brands_list': brands_list,
                       'brands_list_design': brands_list_design})
...
def show_cart(request):
    ...
    if not flag:
        ...
        return render(request, 'client/client_index.html',
                      {'message': '<script type="text/javascript">alert("你都没有登录，还看个毛线吖，尼码！");</script>', 'flag': flag,
                       'c_num': c_num, 'classes_list': classes_list, 'brands_list': brands_list,
                       'brands_list_design': brands_list_design})
    else:
        cart = client_mongodb_options.find_cart(db, username)
        commoditys_list = []
        ...
        return render(request, 'client/client_order_shopcart.html',
                      {'username': username, 'flag': flag, 'c_num': c_num, 'classes_list': classes_list,
                       'brands_list': brands_list, 'brands_list_design': brands_list_design,
                       'commoditys_list': commoditys_list})
...
def show_order(request):
    ...
    # 获取订单信息
    order_list = client_mongodb_options.find_order(db, 'fanzhengle')
